...
/// vector. A sentinel value of [`NONE`] represents a missing link. A tombstone node has
/// `token_id == NONE`.
#[derive(Clone, Copy, Debug)]
pub(crate) struct Node {
    /// Token currently stored at this position, or [`NONE`] for a tombstone slot.
    pub(crate) token_id: TokenId,
    /// Index of the previous live node, or [`NONE`] if this is the head.
    pub(crate) prev: NodePos,
    /// Index of the next live node, or [`NONE`] if this is the tail.
    pub(crate) next: NodePos,
}

impl Node {
    /// Returns `true` if this slot has been removed by a merge.
    #[inline]
